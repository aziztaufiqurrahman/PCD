...
while True:
    ...
    if event == 'Open':
        ...
        if event == 'Inverse (Negative)':
            ...
            image_element.update(data=encode_img(shown_image))
        elif event == 'Upsample by 1.25':
            shown_image = cv.resize(
                shown_image, (int(shown_image.shape[0] * 1.25), int(shown_image.shape[1] * 1.25)), interpolation=cv.INTER_AREA)

            image_element.update(data=encode_img(shown_image))
            ui_text.update(UI_STRING_FORMAT.format(
                shown_image.shape[0], shown_image.shape[1]))
        elif event == 'Downsample by 1.25':
            shown_image = cv.resize(
